...
namespace Stage.HitDetection
{
    ...
    public static class CapsuleOBBHitChecker
    {
        ...
        public static bool IntersectCapsuleOBB(HitCapsule capsule, OBB obb)
        {
            ...
            // 各軸への検証
            // 1, カプセル線分上の点とOBBの中心点とのベクトルを作成
            // 2, 上記ベクトルをOBBの各軸へ射影した際にできるベクトルを全軸分足す
            // 3, 結果出来た点をOBB内のカプセルへの最近接点とする
            // 4, これで点と線分の最短距離を求めればいい形になるため算出
            // 5, 上記結果がカプセルの半径より短いか確認
            // 6, 結果が非接触の場合、次の点との検証に移る
